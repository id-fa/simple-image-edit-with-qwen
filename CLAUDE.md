...
**Note:** ComfyUI servers (`app_comfyui.py`, `app_comfyui_nunchaku.py`, `app_comfyui_gguf.py`) require a running ComfyUI instance. No torch/diffusers dependency — only flask, requests, pillow, websocket-client. AIO variant requires `Qwen-Rapid-AIO-NSFW-v23.safetensors`, `qwen2.5-vl-7b-instruct-abliterated.safetensors`, and `qwen_image_vae.safetensors`. Nunchaku variant requires `qwen_2.5_vl_7b_fp8_scaled.safetensors`, `qwen_image_vae.safetensors`, and `NunchakuQwenImageLoraStackV3` custom node. GGUF variant requires a GGUF model for UnetLoaderGGUF (e.g. `Qwen-Rapid-NSFW-v23_Q3_K.gguf`), a GGUF CLIP for CLIPLoaderGGUF (e.g. `Qwen2.5-VL-7B-Instruct-heretic.Q3_K_L.gguf`), and `qwen_image_vae.safetensors`. Models are auto-detected from ComfyUI or specified via `--gguf-model`/`--clip-model`. LoRA files in `server/LoRA/` must be in ComfyUI's search path (use `--comfyui-path` for auto-registration).
...
| ComfyUI GGUF node missing | Install GGUF loader nodes (`UnetLoaderGGUF`, `CLIPLoaderGGUF`) in ComfyUI (required by `app_comfyui_gguf.py`) |
