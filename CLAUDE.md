...
**Note:** ComfyUI servers (`app_comfyui.py`, `app_comfyui_nunchaku.py`, `app_comfyui_gguf.py`) require a running ComfyUI instance and `city96/ComfyUI-GGUF` custom node (all variants use GGUF CLIP via `CLIPLoaderGGUF`). No torch/diffusers dependency — only flask, requests, pillow, websocket-client. AIO variant requires `Qwen-Rapid-AIO-NSFW-v23.safetensors`, GGUF CLIP (e.g. `Qwen2.5-VL-7B-Instruct-heretic.Q8_0.gguf` + mmproj), and `qwen_image_vae.safetensors`. Nunchaku variant requires `NunchakuQwenImageLoraStackV3` custom node, GGUF CLIP, and `qwen_image_vae.safetensors`. GGUF variant requires a GGUF model for UnetLoaderGGUF (e.g. `Qwen-Rapid-NSFW-v23_Q8_0.gguf`), GGUF CLIP, and `qwen_image_vae.safetensors`. Models are auto-detected from ComfyUI or specified via `--gguf-model`/`--clip-model`. LoRA files in `server/LoRA/` must be in ComfyUI's search path (use `--comfyui-path` for auto-registration).
...
- CLIP: `CLIPLoaderGGUF` (GGUF CLIP, same as AIO/GGUF variants)
