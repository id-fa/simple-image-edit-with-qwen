...
**Note:** ComfyUI servers (`app_comfyui.py`, `app_comfyui_nunchaku.py`, `app_comfyui_gguf.py`) require a running ComfyUI instance. No torch/diffusers dependency — only flask, requests, pillow, websocket-client. AIO variant requires `Qwen-Rapid-AIO-NSFW-v23.safetensors`, `qwen2.5-vl-7b-instruct-abliterated.safetensors`, and `qwen_image_vae.safetensors`. Nunchaku variant requires `qwen_2.5_vl_7b_fp8_scaled.safetensors`, `qwen_image_vae.safetensors`, and `NunchakuQwenImageLoraStackV3` custom node. GGUF variant requires `Qwen-Rapid-NSFW-v23_Q6_K.gguf` (UnetLoaderGGUF), `Qwen2.5-VL-7B-Instruct-heretic.Q6_K.gguf` (CLIPLoaderGGUF), and `qwen_image_vae.safetensors`. LoRA files in `server/LoRA/` must be in ComfyUI's search path (use `--comfyui-path` for auto-registration).
