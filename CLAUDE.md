...
23. **server/comfyui_workflow/comfyui_flux1klein9b_api.json** - ComfyUI API format workflow for FLUX.2 Klein 9B fp8 (used by app_comfyui_klein.py)
24. **server/comfyui_workflow/enhance_prompt_api.json** - ComfyUI API format workflow for prompt enhancement via QwenVL GGUF (used by all ComfyUI / WaveSpeed servers)
...
python app_comfyui_gguf.py --gguf-model "Qwen-Rapid-NSFW-v23_Q3_K.gguf"
python app_comfyui_gguf.py --clip-model "Qwen2.5-VL-7B-Instruct-heretic.Q3_K_L.gguf"
...
### ComfyUI Web Server — FLUX.2 Klein 9B workflow (API backend)
```powershell
cd server
...
- Prompt enhancement (ComfyUI only): `Enhance` button runs a local VLM (QwenVL GGUF via QwenVL-F_GGUF_Advanced node) to expand short prompts into detailed ones. Uses `enhance_prompt_api.json` workflow. Sends current Img1 and Img2 for context-aware enhancement. Button hidden if workflow file not found. `Undo` button appears after enhancement to revert to pre-enhance prompt; persists across submissions until next Enhance (so user can revert after viewing generation results). Pre-enhance prompt is submitted with the job as `original_prompt` and displayed in gallery history alongside the enhanced prompt. Two separate client-side buffers: `preEnhancePrompt` (Undo button, persists across submissions) and `originalPromptForSubmit` (sent as `original_prompt` on next submit, then cleared) — ensures subsequent non-enhanced submissions do NOT inherit the previous enhance's original_prompt
...
- Gallery login gate: When `--gallery` is enabled, a password login screen is shown first. Login via `POST /api/login` (plaintext in body, not in URL). Server returns a SHA-256 hashed token; all subsequent GET/DELETE APIs authenticate via this token in query parameter (plaintext password never appears in access logs)
...
- Original filename display: Uploaded filenames are saved per job (`input_names` JSON array, img1 lineage only — img2 filenames are not tracked). Displayed only to the uploader (user_hash match). Download filenames are prefixed with the original filename for the owner. When img1 is a gallery reference to a prior job, the base name is inherited from that source job's `input_names[0]` with the source `job_id` appended, so lineage accumulates as `{original_stem}_{jobId1}_{jobId2}...` across iterative edits. Drawing refs and refs to jobs without stored `input_names` do not inherit (result downloads fall back to the generic `result_qwen-edit_{date}_{job_id}.png`). See `_inherit_img1_base_name()` in `server/lib/server_routes.py`
- Download filename format: result downloads include a `qwen-edit` tag and the job's creation date (YYYYMMDD). Pattern: `result_qwen-edit_{YYYYMMDD}_{job_id}.png`, or `{original}_qwen-edit_{YYYYMMDD}_{job_id}.png` for the uploader. Gallery items use the stored `created` timestamp (so old downloads retain their original generation date); the just-finished result area uses today's date. Server-side `/api/result/<job_id>` emits the same name via `send_file(download_name=...)`. Client-side helpers: `ymdFromEpoch(sec)`, `ymdToday()`
- Drawing editor: Click any image (gallery thumbnails, upload previews, generated results) to open a full-screen drawing editor with:
  ...
  - Grid collage button (`+ Grid Collage`): Opens a dedicated collage editor for building a single composite image from multiple sources. Grid layouts: 1×2, 2×1, 2×2, 4×1, 1×4 (rows×cols, base cell size 512px — total canvas size derived from grid, e.g. 2×2 → 1024×1024, 1×4 → 2048×512). Background color selectable (black or white); divider + outer border auto-colored as white on black bg / black on white bg (line width `max(2, min(w,h)/256)`). Per-cell image placement: drop file on cell (or click empty cell → file picker), initial cover-fit, drag body to reposition, drag corner handles to resize (with optional aspect-ratio lock). Image is clipped to its cell's bounds at render. Changing grid layout rescales existing images with cover-fit into the new cell bounds. `Confirm → Edit` renders the composite without UI decorations (dashed active-cell outline, resize handles, "Drop image" placeholder labels) and hands the dataURL to `openDrawingEditor()`, so the collage becomes the BG layer of a drawing session (ready to be drawn over, saved as composite/line via existing Save buttons, and then selected as Img1/Img2). Particularly useful as a reference-image builder for FLUX.2 Klein 9B (`app_comfyui_klein.py`) where multi-ref scenes can be pre-assembled into a single Img1/Img2 input. State lives entirely on the client (`coCells[]`, `coActiveIdx`, `coDragging`, `coResizing`) — no server-side collage storage
...
- Login API: `POST /api/login` — accepts plaintext password, returns `{"ok": true, "token": "<sha256-hash>"}`. Token used for all subsequent GET/DELETE authentication
...
**Note:** ComfyUI servers (`app_comfyui.py`, `app_comfyui_nunchaku.py`, `app_comfyui_gguf.py`, `app_comfyui_klein.py`) require a running ComfyUI instance. Qwen variants (AIO/Nunchaku/GGUF) use GGUF CLIP via `city96/ComfyUI-GGUF` custom node; Klein variant uses a standard `CLIPLoader` with a safetensors text encoder (no GGUF CLIP dependency). No torch/diffusers dependency — only flask, requests, pillow, websocket-client. AIO variant requires `Qwen-Rapid-AIO-NSFW-v23.safetensors`, GGUF CLIP (e.g. `Qwen2.5-VL-7B-Instruct-heretic.Q8_0.gguf` + mmproj), and `qwen_image_vae.safetensors`. Nunchaku variant requires `NunchakuQwenImageLoraStackV3` custom node, `pollockjj/ComfyUI-MultiGPU` custom node (CLIP CPU offload via `CLIPLoaderGGUFMultiGPU`), GGUF CLIP, and `qwen_image_vae.safetensors`. GGUF variant requires a GGUF model for UnetLoaderGGUF (e.g. `Qwen-Rapid-NSFW-v23_Q8_0.gguf`), GGUF CLIP, and `qwen_image_vae.safetensors`. Klein variant requires `flux-2-klein-9b-fp8.safetensors`, `qwen_3_8b_fp8mixed.safetensors` (CLIPLoader with `type=flux2`), and `full_encoder_small_decoder.safetensors` VAE. LoRA uses stock `LoraLoaderModelOnly` nodes (3 chained slots, no custom node dependency). Models are auto-detected from ComfyUI or specified via `--gguf-model`/`--clip-model`. LoRA files in `server/LoRA/` must be in ComfyUI's search path (use `--comfyui-path` for auto-registration).
...
### venv for ComfyUI / WaveSpeed server (no torch/diffusers needed)
```powershell
...
- Model loading: `UnetLoaderGGUF` (GGUF transformer) + `CLIPLoaderGGUF` (GGUF CLIP)
- LoRA: same 3-slot `LoraLoaderModelOnly` chain as AIO (UnetLoaderGGUF → slot1 → slot2 → slot3 → ModelSamplingAuraFlow)
- Model auto-detection: GGUF/CLIP models resolved by priority: `--gguf-model`/`--clip-model` CLI arg > workflow template default > first `.gguf`/`.safetensors` detected in ComfyUI. Selected model names written back to workflow template at startup
- Startup checks: ComfyUI connectivity (60s retry), model availability (auto-detected or CLI-specified), LoRA path registration
...
**ComfyUI API Backend — FLUX.2 Klein 9B** (`server/app_comfyui_klein.py`):
- Same architecture as AIO variant but for the FLUX.2 Klein 9B fp8 workflow
- Workflow template loaded from `server/comfyui_workflow/comfyui_flux1klein9b_api.json`
- Image slot mapping: img1 → LoadImage node 252 (Base — provides output size via `GetImageSize` from `ImageScaleToTotalPixels` 291), img2 → LoadImage node 157 (Reference). img1 is used by both positive and negative branches (via `ConditioningZeroOut` + `ReferenceLatent`), img2 is chained into the positive branch as a second reference
- Prompt set directly on `CLIPTextEncode` node 290 (`text` field); negative prompt uses `ConditioningZeroOut` (no text)
- Sampler: `SamplerCustomAdvanced` + separate `RandomNoise` (286, `noise_seed`) + `BasicScheduler` (288, `steps=4` default) + `KSamplerSelect` (292, `euler`) + `CFGGuider` (273, `cfg=1.0` default). Seed range: `random.randint(0, 2**63 - 1)` to match template magnitude
- LoRA: 3-slot chained `LoraLoaderModelOnly` (UNETLoader → 306 → 307 → 308 → CFGGuider). Each slot has `lora_name` + `strength_model`; unused tail slots are deleted from the workflow and `CFGGuider.model` is rewired to the last active slot (or directly to `UNETLoader` 283 when no LoRAs). Max 3 active LoRAs per generation. Uses only stock ComfyUI nodes (no rgthree dependency)
- Models: UNETLoader 283 (`flux-2-klein-9b-fp8.safetensors`), CLIPLoader 284 (`qwen_3_8b_fp8mixed.safetensors`, `type=flux2`), VAELoader 285 (`full_encoder_small_decoder.safetensors`)
- Alignment: local `W_MULT=16`, `H_MULT=16`, max 2048×2048 (FLUX.2 prefers 16px multiples; ComfyUI's `ImageScaleToTotalPixels` with `resolution_steps=1` rescales again server-side)
...
- Flow: upload image (`POST /api/v3/media/upload/binary` → `download_url`) → submit edit task (JSON: `{images, prompt, seed, size}`) → poll result (`GET /api/v3/predictions/{task_id}`) → download output
- Image constraints: 384-3072px per dimension, 1-6 images per request
- Pre-resize: 1M/2M pixels (local Pillow resize before upload)
- No LoRA support (API limitation)
- No step-by-step progress (`has_preview=False`, `get_total_steps=1`)
- Cancel: local-only (polling loop checks `cancel_requests`, no remote cancel API)
