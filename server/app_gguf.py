...
function openLightbox(url) { openDrawingEditor(url); }
...
async function pauseDrawing() {
  ...
  try {
    ...
    closeDrawingEditor(); updateDrawings();
...
function resumeDrawing(drawingId) {
  ...
  bgImg.onload = function() {
    ...
    ovImg.onload = function() {
      ...
      deHistory = []; deSourceUrl = '/api/drawing/' + drawingId + '/bg' + pw;
      editor.style.display = 'flex'; deDrawSaveState();
    };
...
(function() {
  document.addEventListener('pointerdown', function(e) {
    ...
    e.preventDefault(); deIsDrawing = true; deDrawSaveState();
    const ctx = c.getContext('2d');
    const p = deGetCoords(e);
    ctx.beginPath(); ctx.moveTo(p.x, p.y);
    ctx.lineCap = 'round'; ctx.lineJoin = 'round'; ctx.lineWidth = deLineWidth;
    if (deCurrentTool === 'pen') { ctx.globalCompositeOperation = 'source-over'; ctx.strokeStyle = deCurrentColor; }
    else if (deCurrentTool === 'eraser') { ctx.globalCompositeOperation = 'destination-out'; ctx.strokeStyle = 'rgba(0,0,0,1)'; }
    else if (deCurrentTool === 'overwrite') { ctx.globalCompositeOperation = 'source-over'; ctx.strokeStyle = '#ffffff'; }
  });
  ...
  document.addEventListener('pointerup', function() { deIsDrawing = false; });
})();
...
async function saveDrawingToServer(type) {
  ...
  tmp.width = bgCanvas.width; tmp.height = bgCanvas.height;
  const tmpCtx = tmp.getContext('2d');
  if (type === 'composite') { tmpCtx.drawImage(bgCanvas, 0, 0); tmpCtx.drawImage(drawCanvas, 0, 0); }
  else { tmpCtx.fillStyle = '#ffffff'; tmpCtx.fillRect(0, 0, tmp.width, tmp.height); tmpCtx.drawImage(drawCanvas, 0, 0); }
...
function updateDrawings() {
  const pw = sessionPassword ? '?password=' + encodeURIComponent(sessionPassword) : '';
  fetch('/api/drawings' + pw).then(r => r.json()).then(data => {
    if (data.error) return; renderDrawings(data.items || []);
  }).catch(() => {});
}
...
function renderDrawings(items) {
  ...
  for (const item of items) {
    ...
    if (isDraft) {
      html += '<div class="drawing-card draft">'
        ...
        + '<div class="drawing-actions"><button onclick="deleteDrawing(\'' + item.drawing_id + '\')">Del</button></div></div>';
    } else {
      ...
      html += '<div class="drawing-card">'
        ...
        + '<button onclick="deleteDrawing(\'' + item.drawing_id + '\')">Del</button></div>'
  ...
  function uncheckT2i() { const t2i = document.getElementById('t2i'); if (t2i && t2i.checked) { t2i.checked = false; t2i.dispatchEvent(new Event('change')); } }
...
function deleteDrawing(drawingId) {
  ...
  fetch('/api/drawing/' + drawingId + pw, {method: 'DELETE'})
    .then(r => r.json()).then(data => { if (data.ok) updateDrawings(); else alert(data.error || 'Error'); })
    .catch(() => alert('Error'));
}
