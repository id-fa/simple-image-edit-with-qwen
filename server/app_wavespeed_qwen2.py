#!/usr/bin/env python3
"""
Web server for Qwen-Image-Edit using WaveSpeed AI cloud API.
...
WAVESPEED_EDIT_URL = f"{WAVESPEED_BASE_URL}/wavespeed-ai/qwen-image-2.0-pro/edit"

# WaveSpeed image constraints
MIN_DIM = 384
MAX_DIM = 3072
...
def wavespeed_submit_edit(
    ...
    task_id = data.get("id")
    poll_url = data.get("urls", {}).get("get", f"{WAVESPEED_BASE_URL}/predictions/{task_id}")
    if not task_id:
...
def preprocess_image(img: Image.Image, pre_resize_target: int | None) -> Image.Image:
    """Pre-resize and clamp to WaveSpeed dimension constraints."""
    ...
    # Scale up to MIN_DIM if needed
    w, h = img.size
    if w < MIN_DIM or h < MIN_DIM:
        scale = max(MIN_DIM / w, MIN_DIM / h)
        w, h = int(w * scale), int(h * scale)
        img = img.resize((w, h))
        print(f"[info] scaled up to min dim: {w}x{h}", file=sys.stderr)
...
def worker_loop():
    while True:
        ...
        try:
            ...
            # Build size string from first image dimensions
            size_str = None
            if job["t2i"]:
                size_str = f"{T2I_SIZE[0]}*{T2I_SIZE[1]}"
            ...
            print(f"[info] WaveSpeed task submitted: {task_id}", file=sys.stderr)
...
def main():
    ...
    ap = argparse.ArgumentParser(description="Qwen Image Edit Web Server (WaveSpeed AI API)")
    ...
    common.model_info["transformer"] = "Qwen-Image-2.0-Pro (Edit)"
