#!/usr/bin/env python3
"""
Web server for Qwen-Image-Edit using ComfyUI API backend.
...
Usage:
  ...
  python app_comfyui.py --comfyui-path D:\\ComfyUI   # auto-register LoRA path
...
# =======================
_WORKFLOW_PATH = Path(__file__).parent / "comfyui_workflow" / "comfyui_qwen_image_edit_AIO_v23_api.json"
try:
...
WF_NODE = {
    ...
    "unet_loader":      "439",       # UNETLoader
    "clip_loader":      "438",       # CLIPLoader
    ...
    "lora_loader1":     "445",       # LoraLoaderModelOnly (1st, model <- unet_loader)
...
def comfyui_wait_for_result(prompt_id: str, job_id: str, timeout: float = 600) -> dict | None:
    ...
    Returns output dict or None on timeout/cancel.
    Uses WebSocket for progress; falls back to polling.
    """
    if HAS_WEBSOCKET:
...
YAML_COMMENT = "# Auto-added by app_comfyui.py — LoRA path for simple-image-edit server"
...
def reboot_comfyui_and_wait(timeout: float = 120) -> bool:
    ...
    Requires ComfyUI-Manager extension. Returns True if reboot succeeded.
    """
    print("[info] ComfyUIを再起動しています... / Rebooting ComfyUI...", file=sys.stderr)
    try:
        http_requests.get(f"{comfyui_url}/manager/reboot", timeout=5)
    except Exception:
        # Connection reset/refused is expected — server is restarting
        pass

    # Wait for ComfyUI to come back
    start = time.time()
    while time.time() - start < timeout:
        time.sleep(2)
        try:
...
def scan_lora_folder(comfyui_lora_names: list[str]) -> list[dict]:
    ...
    for f in sorted(LORA_DIR.iterdir()):
        ...
        # Find matching ComfyUI name (may include subfolder prefix)
        comfyui_name = None
        for cn in comfyui_lora_names:
            # Match by exact filename or trailing path component
...
def build_workflow(image1_name: str, image2_name: str | None, prompt: str,
    ...
    if pre_resize:
        megapixels = pre_resize / 1_000_000  # 300000 -> 0.3, 1000000 -> 1.0
    else:
        megapixels = 1.0
    ...
    # Chain: UNETLoader(439) -> 445 -> 451 -> 452 -> ModelSamplingAuraFlow(441)
    ...
    # Determine which loader nodes to keep vs remove
    last_model_source = [N["unet_loader"], 0]  # default: direct from UNETLoader
...
def main():
    ...
    ap = argparse.ArgumentParser(description="Qwen Image Edit Web Server (ComfyUI API backend)")
    ...
    # If YAML was modified, reboot ComfyUI so it picks up the new LoRA path
    if yaml_modified:
        if reboot_comfyui_and_wait():
            # Re-fetch available models after reboot
            available = comfyui_get_available_models()
        else:
    ...
    model_info["pipeline"] = f"ComfyUI ({comfyui_url})"
    model_info["transformer"] = unet_name
