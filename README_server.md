...
- 必要なモデル（ComfyUI の `models/LLM/jwest33/qwen3.5-9b-null-space-abliterated-GGUF/` に配置）:
...
フォーム内の **+ Grid Collage** ボタンで、複数画像を1枚に連結合成するコラージュエディタを開けます。合成結果はそのまま描画エディタで編集し、Img1/Img2として利用可能です。

**特に FLUX.2 Klein 9B (`app_comfyui_klein.py`) のリファレンス画像**として有用です。複数のキャラクター・ポーズ・背景を1枚にまとめた参照画像を簡単に構築できます。

#### グリッドレイアウト

| ボタン | 行×列 | 合成サイズ |
|--------|-------|-----------|
| **1×2** | 1行×2列（横並び） | 1024×512 |
| **2×1** | 2行×1列（縦並び） | 512×1024 |
| **2×2** | 2行×2列 | 1024×1024 |
| **4×1** | 4行×1列（縦4段） | 512×2048 |
| **1×4** | 1行×4列（横4連） | 2048×512 |

1セル固定 512×512 px。合計サイズはグリッド形状に応じて可変。

#### 背景色と枠線

- **Black / White** ボタンで背景色を切り替え
- 枠線は自動で反対色（BG黒→白枠、BG白→黒枠）。外周と内部のセル境界線が描画される

#### セルへの画像配置

| 操作 | 動作 |
|------|------|
| 画像ファイルをセルにドロップ | そのセルに画像を配置（初期はcover-fitでセル全体を埋める） |
| 空セルをクリック | ファイルピッカーが開き、選択したファイルをそのセルに配置 |
| 配置済み画像本体をドラッグ | 位置を移動。セル外にはみ出した領域は消える |
| 四隅の紫ハンドルをドラッグ | サイズ変更 |
| **Lock ratio** チェックON時のリサイズ | アスペクト比を固定したままサイズ変更 |
| グリッドを変更 | 配置済み画像は新しいセル寸法に合わせてcover-fitで再配置される |

アクティブセルは紫の点線で強調表示されます。

#### ボタン

| ボタン | 説明 |
|--------|------|
| **Clear cell** | 選択中のセルの画像を削除 |
| **Clear all** | 全セルの画像を削除（確認ダイアログ付き） |
| **Confirm → Edit** | 合成画像を生成し、描画エディタで開く（BGレイヤーにセット） |
| **Cancel** | コラージュエディタを閉じる（Escキーでも可） |

#### 合成後のワークフロー

1. `Confirm → Edit` を押すと、描画エディタが合成画像をBGレイヤーにして開く
2. ペン等で追記描画が可能
3. **Save(+bg)** で合成画像＋描画を1枚のPNGとして保存
4. 保存後、drawingsサムネイルに表示され、**Img1** / **Img2** ラジオボタンで次の生成の入力として使用可能
...
- Required models (place in ComfyUI `models/LLM/jwest33/qwen3.5-9b-null-space-abliterated-GGUF/`):
  - `qwen3.5-9b-null-space-abliterated-q8_0.gguf`
  - `mmproj-qwen3.5-9b-f16.gguf`
...
The **+ Grid Collage** button in the form opens a dedicated collage editor that composes multiple images into a single composite. The result flows into the drawing editor and can then be used as Img1/Img2.

**Especially useful as a reference image builder for FLUX.2 Klein 9B (`app_comfyui_klein.py`)** — pack multiple characters, poses, or backgrounds into a single reference frame in seconds.

#### Grid Layouts

| Button | Rows × Cols | Composite Size |
|--------|-------------|----------------|
| **1×2** | 1 row × 2 cols (horizontal pair) | 1024×512 |
| **2×1** | 2 rows × 1 col (vertical pair) | 512×1024 |
| **2×2** | 2 rows × 2 cols | 1024×1024 |
| **4×1** | 4 rows × 1 col (tall strip) | 512×2048 |
| **1×4** | 1 row × 4 cols (wide strip) | 2048×512 |

Base cell size is fixed at 512×512 px; total canvas size follows the grid shape.

#### Background & Borders

- Switch background color with the **Black / White** buttons
- Border color auto-switches to the opposite (black bg → white borders, white bg → black borders). Both the outer frame and the inter-cell dividers are drawn.

#### Placing Images in Cells

| Action | Result |
|--------|--------|
| Drop an image file on a cell | The image is placed in that cell with an initial cover-fit (fills the cell) |
| Click an empty cell | File picker opens; the chosen file is placed in that cell |
| Drag the image body | Reposition. Content outside the cell is clipped away |
| Drag a purple corner handle | Resize |
| Resize with **Lock ratio** checked | Resize while preserving the aspect ratio |
| Change grid layout | Existing images are re-cover-fit into the new cell bounds |

The active cell is highlighted with a purple dashed outline.

#### Buttons

| Button | Description |
|--------|-------------|
| **Clear cell** | Remove the image from the active cell |
| **Clear all** | Remove images from all cells (with a confirm dialog) |
| **Confirm → Edit** | Render the composite and open it in the drawing editor as the BG layer |
| **Cancel** | Close the collage editor (Esc also works) |

#### Post-composition Workflow

1. `Confirm → Edit` opens the drawing editor with the composite as its BG layer
2. You can draw on top with any of the drawing tools
3. **Save(+bg)** saves the composite + overlay as a single PNG
4. The saved drawing appears in the Drawings list and can be selected as **Img1** / **Img2** for the next generation
