...
5. **Pre-resize** で入力画像のサイズを選択（0.3M / 1M pixels）。Pre-resizeは結果にほとんど影響しないため、通常はデフォルトの0.3Mから変更する必要はありません
6. **Generate** ボタンをクリック
...
5. Select input image size in **Pre-resize** (0.3M / 1M pixels). Pre-resize has minimal impact on results, so there is usually no need to change from the default 0.3M
6. Click **Generate**
...
Specify with the `--lora` option at startup (repeatable).
